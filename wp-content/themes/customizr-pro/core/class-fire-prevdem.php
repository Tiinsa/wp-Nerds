...
if ( ! class_exists( 'CZR_prevdem' ) ) :
  final class CZR_prevdem {
    ...
    function czr_fn_set_fp_title( $text, $fp_single_id, $featured_page_id ) {
      switch ($fp_single_id) {
        case 'one':
          $text = __( 'Who We Are', 'customizr-pro');
          break;

        case 'two':
          $text = __( 'What We Do', 'customizr-pro');
          break;

        case 'three':
          $text = __( 'Contact Us', 'customizr-pro');
          break;
      }
      return $text;
    }
    ...
    */
    ...
    function czr_fn_set_demo_slide_data( $data, $slider_name_id, $id ) {
        ...
        switch ( $id ) {
          ...
          case 2 :
            $data['title']        = __( 'The Customizr theme fits nicely on any mobile devices.', 'customizr-pro' );
            $data['link_url']     = 'javascript:void(0)';
            $data['button_text']  = '';//__( 'Call to action' , 'customizr');
          break;

          case 3 :
            $data['title']        = __( 'Engage your visitors with a carousel in any pages.', 'customizr-pro' );
            $data['link_url']     = 'javascript:void(0)';
            $data['button_text']  = __( 'Call to action' , 'customizr-pro');
          break;
        };
        ...
        case 'right':
          $_widgets_to_print[] = array(
            'WP_Widget_Search' => array(
              'instance' => array(
                'title' => __( 'Search', 'customizr-pro')
              ),
              'args' => $default_args
            ),
            'WP_Widget_Recent_Posts' => array(
              'instance' => array(
                'title' => __( 'Recent Posts', 'customizr-pro'),
                'number' => 6
              ),
              'args' => $default_args
            ),
            'WP_Widget_Recent_Comments' => array(
              'instance' => array(
                'title' => __( 'Recent Comments', 'customizr-pro'),
                'number' => 4
              ),
              'args' => $default_args
            )
