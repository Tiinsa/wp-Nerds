...
if( !class_exists( 'header_text_image' ) ) :
class header_text_image extends WP_Widget
{
    function header_text_image() {
        $widget_ops = array( 
            'classname' => 'header-text-image', 
            'description' => __( 'Arbitrary text or HTML, with a simple image above text. This widget must be used only in Header Sidebar', 'yit' )
        );

        $control_ops = array( 'id_base' => 'header-text-image', 'width' => 430 );

        $this->WP_Widget( 'header-text-image', __( 'Header Text With Image', 'yit' ), $widget_ops, $control_ops );      
    ...
    function widget( $args, $instance ) {
		...
		$text .= '</div><div class="text-content">' . $title . '<p>' . $instance['text'] . '</p></div>';
